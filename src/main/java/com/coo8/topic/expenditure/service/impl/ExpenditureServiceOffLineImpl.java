...
import com.coo8.topic.expenditure.dao.ExpenditureOffLineMapper;
...
@Service
@Slf4j
public class ExpenditureServiceOffLineImpl implements ExpenditureOffLineService{
	@Resource
	private ExpenditureOffLineMapper expenditureOffLineMapper;
	private SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
	@Override
	@Transactional(readOnly=true)
	public ExpenditureDto findExpenditure(int id) {
		log.info("findExpenditure is start!");
		Expenditure expd = expenditureOffLineMapper.getExpenditureById(id);
		...
		if(expd.getPortId() >0){
			List<PortType> portTypeList = expenditureOffLineMapper.getPortTypeList(expd.getPortId());
		...
		if(expd.getStairChanId() >0){
			 List<ChannelDetailStair> stairList = expenditureOffLineMapper.getChannelStairList(expd.getStairChanId(),0,-1);
			 if(stairList.size()>0) stair = stairList.get(0);
		}else{
			stair.setStairName("-");
		}
		if(expd.getStairChanId() >0 && expd.getSecondChanId() >0){
					List<ChannelDetailSecond> secondList = expenditureOffLineMapper.getChannelSecondList(expd.getSecondChanId(),0,-1);
		...
		if(expd.getSecondChanId() >0 && expd.getThirdChanId() >0){
			 List<ChannelDetailThird> thirdList = expenditureOffLineMapper.getChannelThirdList(expd.getThirdChanId(),0,0,-1);
	...
	public int addExpenditure(Expenditure entity) {
		log.info("addExpenditure is start!");
		 return expenditureOffLineMapper.insertExpenditureDetail(entity);
	}

	@Override
	@Transactional(readOnly=true)
	public List<ExpenditureDto> findExpenditureList(Expenditure serach) {
		 List<Expenditure> expenditureList = expenditureOffLineMapper.getExpenditureList(serach);
		 if(!expenditureList.isEmpty()){
			 ...
			 for (Expenditure expenditure : expenditureList) {
				...
				if(expenditure.getPortId() >0){
					List<PortType> portTypeList = expenditureOffLineMapper.getPortTypeList(expenditure.getPortId());
					if(portTypeList.size()>0){
						PortType portType = portTypeList.get(0);
						dto.setPort(portType.getPortName());
					}
				...
				if(expenditure.getStairChanId() >0){
					 List<ChannelDetailStair> stairList = expenditureOffLineMapper.getChannelStairList(expenditure.getStairChanId(),0,-1);
					 if(stairList.size()>0) stair = stairList.get(0);
				}else{
					stair.setStairName("-");
				}
				if(expenditure.getStairChanId() >0 && expenditure.getSecondChanId() >0){
							List<ChannelDetailSecond> secondList = expenditureOffLineMapper.getChannelSecondList(expenditure.getSecondChanId(),0,-1);
							if(secondList.size()>0) second = secondList.get(0);
				...
				if(expenditure.getSecondChanId() >0 && expenditure.getThirdChanId() >0){
					 List<ChannelDetailThird> thirdList = expenditureOffLineMapper.getChannelThirdList(expenditure.getThirdChanId(),0,0,-1);
					 if(thirdList.size()>0)  third = thirdList.get(0);
				}else{
					third.setThirdName("-");
				}
	...
	public List<ExportExpenditureDto> exportExpenditureListByDate(String startDate, String endDate) {
		...
		DateFormat dateFormat = new SimpleDateFormat("yyyy/MM/dd");
		 List<Expenditure> list = expenditureOffLineMapper.getExpenditureListByDate(startDate, endDate);
		 if(list !=null && list.size()>0){
			 ...
			 for (Expenditure expenditure : list) {
				...
				if(expenditure.getPortId() >0){//端口
					List<PortType> portList = expenditureOffLineMapper.getPortTypeList(expenditure.getPortId());
...
//				expenditure.setCreateDate(date);
				...
				addCount+= this.expenditureOffLineMapper.insertExpenditureDetail(expenditure);
			}
		}
		return addCount;
	}
		...
		if(dto.getPortId()!=0){
			IDGenerator idGenerator = IDGeneratorUtils.getInstance();
					...
					if(StringUtils.isNotBlank(dto.getStairName())){
						...
						int stairNum = this.expenditureOffLineMapper.insertChannelDetail_Stair(chanStair);
						...
						if(StringUtils.isNotBlank(dto.getSecondName())){
							...
							int secondSum = this.expenditureOffLineMapper.insertChannelDetail_Second(chanSecond);
						...
						if(StringUtils.isNotBlank(dto.getSecondName()) || dto.getSecondId()>0){
							if(StringUtils.isNotBlank(dto.getThirdName())){
								...
								int thirdSum = this.expenditureOffLineMapper.insertChannelDetail_Third(chanThird);
	...
	public int addPortType(PortType type) {
		int insertNum = expenditureOffLineMapper.insertPortType(type);
		return insertNum;
	}

	@Override
	public List<PortType> findPortTypeList() {
		List<PortType> typeList = expenditureOffLineMapper.getPortTypeList(0);
		return typeList;
	}

	@Override
	public List<ChannelDetailStair> findChannelStairList(int portId) {
		List<ChannelDetailStair> stairList = expenditureOffLineMapper.getChannelStairList(0,portId,0);
		return stairList;
	}

	@Override
	public List<ChannelDetailSecond> findChannelSecondList(long stairId) {
		List<ChannelDetailSecond> secondList = expenditureOffLineMapper.getChannelSecondList(0,stairId,0);
		return secondList;
	}

	@Override
	public List<ChannelDetailThird> findChannelThirdList(long stairId,long secondId) {
		List<ChannelDetailThird> thirdList = expenditureOffLineMapper.getChannelThirdList(0,stairId,secondId,0);
		return thirdList;
	}
	...
	public ChannelDetailStair findChannelStair(String name,int portId) {
		return expenditureOffLineMapper.getChannelStairByName(name.trim(),portId);
	}

	@Override
	@Transactional(readOnly=true)
	public ChannelDetailSecond findChannelSecond(String name,long stairId) {
		return expenditureOffLineMapper.getChannelSecondByName(name.trim(),stairId);
	...
	public ChannelDetailThird findChannelThird(String name,long secondId) {
		return expenditureOffLineMapper.getChannelThirdByName(name.trim(),secondId);
	}
	...
	public int modifyExpenditure(ExpenditureDto dto) {
		...
		expenditureOffLineMapper.updateExpenditure(entity);
		return 0;
	}

	@Override
	public List<ExpendAnalyseBase> getExpenditureAnalyseListByDate(String startDate, String endDate) {
		return expenditureOffLineMapper.getExpenditureAnalyseListByDate(startDate,endDate);
	}

	@Override
	public List<ExpendAnalyseCharge> selectExpendDataInMonth(String yearNum, Integer stairId) {
		return expenditureOffLineMapper.selectExpendDataInMonth(yearNum,stairId);
	}

	@Override
	public List<ExpendPortAnalyseCharge> selectPortExpendDataInMonth(String yearNum, Integer portId) {
		return expenditureOffLineMapper.selectPortExpendDataInMonth(yearNum,portId);
	}

	@Override
	public List<WholeExpendAnalyseCharge> selectWholeExpendDataInMonth(String yearNum) {
		return expenditureOffLineMapper.selectWholeExpendDataInMonth(yearNum);
	}

	@Override
	public BigDecimal selectPortExpendAmount(String startDate, String endDate, Integer portId) {
		return expenditureOffLineMapper.selectPortExpendAmount(startDate,endDate,portId);
	}

	@Override
	public BigDecimal selectWholeExpendAmount(String startDate, String endDate) {
		return expenditureOffLineMapper.selectWholeExpendAmount(startDate,endDate);
	}

	@Override
	public int findPortByName(String name) {
		return this.expenditureOffLineMapper.getPortByName(name.trim());
