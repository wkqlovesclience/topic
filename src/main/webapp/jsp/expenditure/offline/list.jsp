...
<head>
	...
	<script type="text/javascript"  >
        ...
        $(document).ready(function(){
            ...
            if(selectPortIds !=0){
                $.ajax({
                    url: "${ctx}/ExpenditureOffLine/stairList_ajax.action?selectPortIds="+selectPortIds,//请求url
                    type: "post",
                    dataType:"json",
                    async:false,
                    cache:true,
            ...
            if(selectStairIds!=0){
                $.ajax({
                    url: "${ctx}/ExpenditureOffLine/secondList_ajax.action?selectStairIds="+selectStairIds,//请求url
                    type: "post",
                    async:false,
                    cache:true,
                    dataType:"json",
            ...
            if(selectSecondIds!=0){
                $.ajax({
                    url: "${ctx}/ExpenditureOffLine/thirdList_ajax.action?selectStairIds="+selectStairIds+"&selectSecondIds="+selectSecondIds,
                    type: "post",
                    async:false,
                    cache:true,
                    dataType:"json",
        ...
        function tunePage(num) {
            ...
            var url = '${ctx}/ExpenditureOffLine/list.action?pageNum=' + num;
        ...
        function changePageSize(size) {
            ...
            var thirdChanId = document.getElementById("thirdChanId").value;
            var id  = document.getElementById("id").value;
            var url = "${ctx}/ExpenditureOffLine/list.action?pageNum=1&pageSize="+size+"&searchKey.portId="+portId+
        ...
        <!--添加渠道页面-->
        function addChannel(){
            window.location.href = "${ctx}/ExpenditureOffLine/addChannelView.action";
        }
        <!--添加花费页面-->
        function goAddList(){
            window.location.href = "${ctx}/ExpenditureOffLine/addView.action"
        }
        <!--导入数据校验-->
        ...
        <!--导出文件-->
        function exportExpenditureOffLine(){
...
<div class="mod-1">
	...
	<div class="bd clearfix">
		<form action="${ctx}/ExpenditureOffLine/list.action" method="post" id="f1">
						...
						<s:iterator value="expenditureList"  status="st">
							<tr>
								...
								<td><erm:authorize code="topic_expenditure_modify">
									<a href="${ctx}/ExpenditureOffLine/modifyView.action?expenditureId=<s:property value='id'/>"/>修改</a>
								</erm:authorize></td>
		...
		<erm:authorize code="topic_expenditure_download">
			<form action="${ctx}/ExpenditureOffLine/import.action" method="post"  enctype="multipart/form-data" onsubmit="return sureFile()">
